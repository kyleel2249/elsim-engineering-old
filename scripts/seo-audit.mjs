 ...
 *   node scripts/seo-audit.mjs https://www.elsimengineeringlimited.com
...
async function fetchText(path, { redirect = 'follow' } = {}) {
  ...
  const res = await fetch(url, {
    redirect,
    headers: { 'user-agent': 'ELSIM-SEO-Audit/1.0 (+https://www.elsimengineeringlimited.com)' },
  });
...
async function auditSitemap() {
  ...
  // The retired domain must never appear (correct domain: elsimengineeringlimited.com).
  const legacy = locs.filter((u) => {
    try {
      const h = new URL(u).hostname;
      return h === 'elsimengineering.com' || h.endsWith('.elsimengineering.com');
    } catch {
      return false;
    }
  });
  if (legacy.length) {
    fail('sitemap.legacy-domain', 'Sitemap lists the retired elsimengineering.com domain', legacy.slice(0, 3).join(', '));
  } else {
    pass('sitemap.legacy-domain', 'No retired-domain URLs in sitemap');
  }

  // Every <loc> should be on the host being audited (Search Console rejects cross-host entries).
  const originHost = new URL(origin).hostname;
  const crossHost = locs.filter((u) => {
    try {
  ...
  if (crossHost.length) {
    const detail = `${crossHost.length} of ${locs.length} not on ${originHost}, e.g. ${crossHost[0]}`;
    // A pages.dev preview legitimately advertises the production host; anywhere else it is a real error.
    if (originHost.endsWith('.pages.dev')) warn('sitemap.origin-match', 'Sitemap hosts differ from the preview host', detail);
    else fail('sitemap.origin-match', 'Sitemap hosts do not match the audited host', detail);
  } else {
    pass('sitemap.origin-match', `All sitemap URLs are on ${originHost}`);
  }

  const dupes = locs.filter((u, i) => locs.indexOf(u) !== i);
  if (dupes.length) fail('sitemap.duplicates', 'Sitemap has duplicate URLs', dupes.slice(0, 3).join(', '));
  else pass('sitemap.duplicates', 'No duplicate URLs in sitemap');

  return locs;
}

/**
 * Every URL a sitemap lists should answer 200 directly. A 3xx (for example
 * `/about` -> `/about/`) shows up in Search Console as "Page with redirect".
 */
async function auditSitemapUrls(locs) {
  if (!locs.length) return;
  const problems = [];
  for (const loc of locs) {
    try {
      const target = new URL(loc);
      // Fetch the same path on the audited origin so preview hosts can be checked too.
      const url = `${origin}${target.pathname}${target.search}`;
      const res = await fetch(url, {
        redirect: 'manual',
        headers: { 'user-agent': 'ELSIM-SEO-Audit/1.0' },
      });
      if (res.status !== 200) {
        problems.push(`${res.status} ${target.pathname}${res.headers.get('location') ? ` -> ${res.headers.get('location')}` : ''}`);
      }
    } catch (error) {
      problems.push(`error ${loc}: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  if (problems.length) {
    fail('sitemap.urls', `${problems.length} of ${locs.length} sitemap URLs do not return 200`, problems.slice(0, 5).join('; '));
  } else {
    pass('sitemap.urls', `All ${locs.length} sitemap URLs return 200 without redirects`);
  }
}
